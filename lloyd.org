...
`after-init-hook`.
* Global Properties
We can set up some of the headers for org-mode tangling to new
defaults. There's no code for exposition (yet), so turn tangling on by
default.

'noweb' support is enabled; there's no Lisp construct that would
confuse it, so might as well.
